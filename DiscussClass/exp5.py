...
def generateLines(filePath):
    ...
    listSex = ['男', '女']
    listMail = ['.com', '.org', '.edu', '.cn', '.net']
    listEnchars = 'qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM'

    with open(filePath,'w',encoding="UTF-8") as f:
        for i in range(234):
            namepart1 = choice(listXing)
            namepart2 = ''.join([choice(listMing) for i in range(0,randint(1,3))])
            fullname = namepart1 + namepart2
            theSex = choice(listSex)
            theAge = choice(range(18,100))
            theAddress = ''.join([choice(listMing) for i in range(0,randint(9,30))])
            thePhoneNumber = ''.join(choice("0123456789") for i in range(11))
            theMailpart1 = ''.join([choice(listEnchars) for i in range(randint(5,10))])
            theMailpart2 = ''.join([choice(listEnchars) for j in range(randint(5,10))])
            theMailpart3 = choice(listMail)
            theMail = theMailpart1 + '@' +theMailpart2 + theMailpart3

            fullInfo = fullname + ',' + theSex + ',' + str(theAge) + ',' + thePhoneNumber + ',' + theAddress + ',' + theMail
...
if __name__ == "__main__":
    generateLines("DiscussClass\\exp5.txt")
    readFile("DiscussClass\\exp5.txt")